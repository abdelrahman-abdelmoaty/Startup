...
export default function PricingPlanCard({
  pricingPlan: { name, desc, pricePerMonth, pricePerYear, features },
  yearly,
}: {
        ...
        {PRICING_PLANS_FEATURES.map((ppr, idx) => {
          return (
            <div key={idx} className="flex items-center gap-2">
              <div className="bg-[#ECF0FF] dark:bg-[#212957] p-1 rounded-full">
                {features.includes(idx) ? (
                  <CheckMarkSVG className="text-foreground" />
                ) : (
